...
expName = 'word_priming_exp'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Dell\\OneDrive\\Desktop\\PSY310\\6 word priming\\word_priming_exp_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    for thisTrial_2 in trials_2:
        ...
        # create an object to store info about Routine test_2
        test_2 = data.Routine(
            name='test_2',
            components=[Test_Text, textbox, submit_text, test_mouse],
        )
        test_2.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        ...
        # store start times for test_2
        test_2.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        test_2.tStart = globalClock.getTime(format='float')
        test_2.status = STARTED
        thisExp.addData('test_2.started', test_2.tStart)
        test_2.maxDuration = None
        # keep track of which components have finished
        test_2Components = test_2.components
        for thisComponent in test_2.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "test_2" ---
        test_2.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 10.0:
            ...
            # if test_mouse is stopping this frame...
            if test_mouse.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > test_mouse.tStartRefresh + 10-frameTolerance:
                    # keep track of stop time/frame for later
                    test_mouse.tStop = t  # not accounting for scr refresh
                    test_mouse.tStopRefresh = tThisFlipGlobal  # on global time
                    test_mouse.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.addData('test_mouse.stopped', t)
                    # update status
                    test_mouse.status = FINISHED
                ...
                if buttons != prevButtonState:  # button state changed?
                    ...
                    if sum(buttons) > 0:  # state changed to a new click
                        ...
                        if not gotValidClick:
                            test_mouse.clicked_name.append(None)
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                test_2.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in test_2.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        for thisComponent in test_2.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for test_2
        test_2.tStop = globalClock.getTime(format='float')
        test_2.tStopRefresh = tThisFlipGlobal
        thisExp.addData('test_2.stopped', test_2.tStop)
        trials_2.addData('textbox.text',textbox.text)
        # store data for trials_2 (TrialHandler)
        trials_2.addData('test_mouse.x', test_mouse.x)
        trials_2.addData('test_mouse.y', test_mouse.y)
        trials_2.addData('test_mouse.leftButton', test_mouse.leftButton)
        trials_2.addData('test_mouse.midButton', test_mouse.midButton)
        trials_2.addData('test_mouse.rightButton', test_mouse.rightButton)
        trials_2.addData('test_mouse.time', test_mouse.time)
        trials_2.addData('test_mouse.clicked_name', test_mouse.clicked_name)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if test_2.maxDurationReached:
            routineTimer.addTime(-test_2.maxDuration)
        elif test_2.forceEnded:
            routineTimer.reset()
        else:
